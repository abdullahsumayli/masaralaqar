...
for (const test of testMessages) {
  ...
  if (test.expected.intent && intent !== test.expected.intent) {
    testPassed = false
    failures.push(`Intent: expected "${test.expected.intent}", got "${intent}"`)
  }
  ...
  if (test.expected.propertyType && propertyType !== test.expected.propertyType) {
    testPassed = false
    failures.push(`PropertyType: expected "${test.expected.propertyType}", got "${propertyType}"`)
  }

  if (test.expected.bedrooms && bedrooms !== test.expected.bedrooms) {
    testPassed = false
    failures.push(`Bedrooms: expected ${test.expected.bedrooms}, got ${bedrooms}`)
  }
