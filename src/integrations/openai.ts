...
export class OpenAIService {
  ...
  static async generateSmartReply(
    ...
    const systemPrompt = `أنت "${context.agentName || "مساعد MQ"}"، مساعد عقاري ذكي في السعودية.
...
مهم: أجب بـ 2-4 أسطر كحد أقصى. كن مختصراً ومفيداً.`;
  ...
  private static generateBasicReply(message: string): string {
    ...
    switch (intent) {
      case "greeting":
        return "السلام عليكم ورحمة الله وبركاته! 👋 أهلاً بك في MQ. كيف يمكنني مساعدتك؟";
      case "search":
...
النية:`;
        ...
        if (result) {
          ...
          if (
            ["search", "greeting", "schedule", "contact", "inquire"].includes(
              intent,
            )
          ) {
            return intent;
   ...
   */
  static async extractBudgetWithAI(
    message: string,
   ...
   */
  static async extractPropertyTypeWithAI(
    message: string,
  ): Promise<string | null> {
    try {
  ...
  static async generateReply(
    ...
    try {
      ...
      switch (intent) {
        ...
        case "schedule":
          response =
            "تم تسجيل طلب الزيارة. سيتواصل معك فريقنا قريباً لتأكيد الموعد.";
          break;
   ...
   */
  static async analyzeMessageComprehensive(
    message: string,
