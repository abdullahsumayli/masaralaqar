...
export class LeadRepository {
   ...
   * Create a new lead
   */
  static async createLead(tenantId: string, leadData: Partial<Lead>): Promise<Lead | null> {
    try {
   ...
   */
  static async findLeadByPhone(tenantId: string, phone: string): Promise<Lead | null> {
    try {
      const { data, error } = await supabase
        .from('leads')
        .select('*')
