...
import { BarChart3, Users, Building2, MessageSquare, Loader2, TrendingUp, Activity, Database } from 'lucide-react'
...
export default function AdminSystemAnalyticsPage() {
  ...
  const metricsCards = [
    { label: 'المكاتب المسجّلة', value: stats?.totalOffices || 0, icon: Building2, color: 'text-primary', bg: 'bg-primary/10' },
    { label: 'الاشتراكات الفعّالة', value: stats?.totalSubscriptions || 0, icon: TrendingUp, color: 'text-green-400', bg: 'bg-green-500/10' },
    { label: 'إجمالي العقارات', value: stats?.totalProperties || 0, icon: Database, color: 'text-purple-400', bg: 'bg-purple-500/10' },
    { label: 'إجمالي العملاء', value: stats?.totalLeads || 0, icon: Users, color: 'text-amber-400', bg: 'bg-amber-500/10' },
    { label: 'رسائل AI اليوم', value: stats?.todayAiMessages || 0, icon: MessageSquare, color: 'text-cyan-400', bg: 'bg-cyan-500/10' },
    { label: 'رسائل واتساب اليوم', value: stats?.todayWhatsappMessages || 0, icon: Activity, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
  ]

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white font-cairo">تحليلات النظام</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {metricsCards.map((card, idx) => (
          <div key={idx} className="bg-[#0D1117] border border-[#21262d] rounded-2xl p-6">
      ...
      <div className="bg-[#0D1117] border border-[#21262d] rounded-2xl p-6">
        <h3 className="text-base font-bold text-white mb-4">حالة اتصالات واتساب</h3>
        <div className="flex items-center gap-6">
          <div className="flex-1">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-gray-400">الجلسات المتصلة</span>
              <span className="text-white font-medium">{stats?.connectedSessions || 0} / {stats?.whatsappSessions || 0}</span>
            </div>
      ...
      {/* Recent Offices */}
      <div className="bg-[#0D1117] border border-[#21262d] rounded-2xl p-6">
        <h3 className="text-base font-bold text-white mb-4">آخر المكاتب المسجّلة</h3>
        {stats?.recentOffices && stats.recentOffices.length > 0 ? (
          <div className="space-y-3">
            {stats.recentOffices.map((office, idx) => (
              <div key={office.id} className="flex items-center justify-between p-3 bg-[#161b22] rounded-xl">
