...
export default function KnowledgeBlogPage() {
  ...
  const filteredArticles = articles.filter((article) => {
    const matchesSearch =
      article.title.includes(searchQuery) || article.excerpt.includes(searchQuery);
              ...
              {filteredArticles.map((article) => (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <Link href={`${BLOG_BASE}/${article.slug}`}>
                      <h2 className="text-lg font-bold mb-2 mt-3 group-hover:text-primary transition-colors line-clamp-2">
                        {article.title}
                      </h2>
                    </Link>
                    ...
                    <div className="flex items-center justify-between text-text-muted text-sm mb-4">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {article.reading_time || article.readingTime || 5} دقائق
                        </span>
                      </div>
                    </div>
                    <Link
