...
const resources = [
  { title: "دليل الوسيط العقاري الشامل 2026", description: "كل ما تحتاج معرفته للنجاح في الوساطة العقارية", type: "ebook", price: "premium", priceValue: 199, pages: 85, downloads: 1200 },
  { title: "قائمة تدقيق إغلاق الصفقات", description: "قائمة شاملة لضمان عدم نسيان أي خطوة", type: "template", price: "free", priceValue: 0, pages: 5, downloads: 3500 },
  { title: "كيف تكتب إعلاناً عقارياً جذاباً", description: "دليل خطوة بخطوة لكتابة إعلانات عقارية فعّالة", type: "guide", price: "free", priceValue: 0, pages: 12, downloads: 2800 },
  { title: "إنفوجرافيك: رحلة العميل العقاري", description: "تصور بصري لمراحل تعامل العميل مع الوسيط", type: "infographic", price: "free", priceValue: 0, pages: 1, downloads: 4200 },
  { title: "نموذج عقد وساطة عقارية", description: "نموذج عقد وساطة قابل للتعديل ومتوافق مع النظام", type: "template", price: "premium", priceValue: 49, pages: 8, downloads: 980 },
  { title: "دليل التسويق الرقمي للعقارات", description: "استراتيجيات التسويق العقاري على المنصات الرقمية", type: "guide", price: "premium", priceValue: 149, pages: 45, downloads: 650 },
  { title: "كتاب أساسيات التثمين العقاري", description: "مبادئ وأسس تقييم العقارات للمبتدئين", type: "ebook", price: "premium", priceValue: 249, pages: 120, downloads: 450 },
  { title: "إنفوجرافيك: أنواع العقارات في السعودية", description: "تصنيف شامل لأنواع العقارات المختلفة", type: "infographic", price: "free", priceValue: 0, pages: 1, downloads: 5600 },
  { title: "نموذج خطة تسويقية لعقار", description: "خطة جاهزة للتعديل لتسويق أي عقار", type: "template", price: "free", priceValue: 0, pages: 10, downloads: 2100 },
];
...
export default function KnowledgeLibraryPage() {
  ...
  const filteredResources = resources.filter((resource) => {
    const matchesSearch =
      resource.title.includes(searchQuery) || resource.description.includes(searchQuery);
        ...
        <div className="max-w-6xl mx-auto text-center">
          <motion.div initial="hidden" animate="visible" variants={fadeInUp}>
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-6">
              مصادر ومراجع تساعدك على <span className="text-primary">النجاح في العقار</span>
            </h1>
            <p className="text-xl text-text-secondary max-w-2xl mx-auto mb-10">
              كتب إلكترونية، أدلة، نماذج، وإنفوجرافيك — مصممة للوسيط العقاري السعودي
            </p>
          </motion.div>
        </div>
      </section>
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${
                    activeType === filter.id ? "bg-primary text-white" : "bg-surface text-text-secondary hover:bg-primary/10"
                  }`}
                >
            ...
            {priceFilters.map((filter) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  activePrice === filter.id ? "bg-secondary text-white" : "bg-surface text-text-secondary hover:bg-secondary/10"
                }`}
              >
                {filter.label}
              </button>
              ...
              return (
                ...
                >
                  <div className="aspect-[4/3] bg-gradient-to-br from-primary/10 to-secondary/10 relative flex items-center justify-center">
                    {TypeIcon && <TypeIcon className="w-16 h-16 text-primary/30" />}
      ...
      <section className="py-20 px-4 bg-gradient-to-br from-primary to-primary-dark text-white">
        <div className="max-w-3xl mx-auto text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp}>
            ...
            <h2 className="text-3xl font-bold mb-4">احصل على موارد حصرية في بريدك</h2>
            <p className="text-white/80 text-lg mb-8">اشترك في نشرتنا البريدية واحصل على كتب وأدلة حصرية مجاناً</p>
            <form className="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto">
              ...
              />
              <button type="submit" className="px-8 py-3 bg-secondary text-white rounded-xl font-medium hover:bg-secondary-dark transition-colors">
                اشترك الآن
              </button>
            </form>
            <p className="text-white/60 text-sm mt-4">لن نرسل لك إلا محتوى قيّم — بدون إزعاج</p>
          </motion.div>
        </div>
      </section>
      <section className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { title: "محتوى محلي", description: "جميع الموارد مكتوبة بواسطة خبراء سعوديين يفهمون السوق المحلي" },
              { title: "تحديث مستمر", description: "نحدث مواردنا باستمرار لتواكب تطورات السوق والأنظمة" },
              { title: "تطبيق عملي", description: "كل مورد يحتوي على خطوات عملية قابلة للتطبيق فوراً" },
            ].map((benefit, index) => (
              <motion.div
                key={index}
                initial="hidden"
