...
import { Bot, Save, Loader2, ArrowRight, Sparkles, Clock, MessageSquare, Globe } from 'lucide-react'
...
const toneOptions = [
  ...
  { value: 'formal', label: 'رسمي جداً', desc: 'ردود رسمية بصيغة احترافية عالية' },
]
...
export default function DashboardAIAgentPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/ai-agents', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      if (res.ok) {
        setSaved(true)
      ...
      <div className="border-b border-[#1E293B] bg-[#0D1526]">
        <div className="max-w-5xl mx-auto px-6 py-5">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <h1 className="text-xl font-bold font-cairo">مساعد الذكاء الاصطناعي</h1>
              <p className="text-gray-400 text-sm">تخصيص شخصية وسلوك مساعد AI الخاص بمكتبك</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        {!agent && (
          <div className="bg-[#111E35] border border-[#4F8EF7]/20 rounded-2xl p-6 text-center">
            ...
            <p className="text-gray-300">لم يتم إعداد مساعد AI بعد. يتم إنشاؤه تلقائياً عند تسجيل المكتب.</p>
            <p className="text-gray-500 text-sm mt-1">تأكد من تشغيل Migration 019 وربط حسابك بمكتب.</p>
          </div>
        )}

        {/* Basic Info */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#111E35] border border-[#1E293B] rounded-2xl p-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">اسم المساعد</label>
              <input
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">رسالة الترحيب</label>
              <textarea
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">وصف المكتب</label>
              <textarea
        ...
        {/* Tone & Language */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-[#111E35] border border-[#1E293B] rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-400 mb-2">نبرة الردود</label>
              <div className="space-y-2">
                {toneOptions.map(opt => (
                  <label key={opt.value} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors border ${form.tone === opt.value ? 'border-[#4F8EF7] bg-[#4F8EF7]/5' : 'border-[#1E293B] hover:border-[#4F8EF7]/30'}`}>
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${form.tone === opt.value ? 'border-[#4F8EF7]' : 'border-gray-600'}`}>
                      {form.tone === opt.value && <div className="w-2 h-2 bg-[#4F8EF7] rounded-full" />}
                    </div>
                    <div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {languageOptions.map(opt => (
                  <label key={opt.value} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors border ${form.language === opt.value ? 'border-[#4F8EF7] bg-[#4F8EF7]/5' : 'border-[#1E293B] hover:border-[#4F8EF7]/30'}`}>
                    ...
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${form.language === opt.value ? 'border-[#4F8EF7]' : 'border-gray-600'}`}>
                      {form.language === opt.value && <div className="w-2 h-2 bg-[#4F8EF7] rounded-full" />}
                    </div>
                    <p className="text-white text-sm">{opt.label}</p>
                  </label>
                ))}
        ...
        {/* Working Hours */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-[#111E35] border border-[#1E293B] rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-5">
          ...
          <div className="grid grid-cols-2 gap-4 max-w-md">
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">بداية الدوام</label>
              <input type="time" value={form.working_hours_start} onChange={e => setForm({ ...form, working_hours_start: e.target.value })} className="w-full bg-[#070B14] border border-[#1E293B] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#4F8EF7]" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">نهاية الدوام</label>
              <input type="time" value={form.working_hours_end} onChange={e => setForm({ ...form, working_hours_end: e.target.value })} className="w-full bg-[#070B14] border border-[#1E293B] rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-[#4F8EF7]" />
            </div>
          </div>
          <p className="text-gray-500 text-xs mt-2">المنطقة الزمنية: توقيت الرياض (GMT+3)</p>
        </motion.div>

        {/* Custom Instructions */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-[#111E35] border border-[#1E293B] rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-5">
            <Bot className="w-5 h-5 text-[#4F8EF7]" />
        ...
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            >
              <div className={`w-5 h-5 bg-white rounded-full absolute top-0.5 transition-all ${form.is_active ? 'right-0.5' : 'right-[26px]'}`} />
            </div>
            <span className="text-sm text-gray-300">{form.is_active ? 'المساعد مفعّل' : 'المساعد معطّل'}</span>
          </label>
          <button
            onClick={handleSave}
