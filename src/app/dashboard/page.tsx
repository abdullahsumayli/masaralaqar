...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchPending = async () => {
      try {
        const res = await fetch("/api/unanswered-questions?status=pending");
        const data = await res.json().catch(() => ({}));
        if (res.ok && data?.success && Array.isArray(data?.questions)) {
          setPendingUnansweredCount(data.questions.length);
        }
      } catch {
        // ignore
      }
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
                ...
                <Link
                  ...
                  className="relative flex flex-col items-center gap-2 p-4 rounded-lg bg-surface hover:bg-primary/5 transition-colors text-center"
                >
                ...
                <Link
                  ...
                  className="flex flex-col items-center gap-2 p-4 rounded-lg bg-surface hover:bg-amber-500/5 transition-colors text-center"
                >
