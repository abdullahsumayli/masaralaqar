    ...
    try {
      ...
      if (payload?.instance && payload?.event) {
            ...
            if (payload.event === "connection.update") {
                ...
                // Use getByInstanceIdAny to find session regardless of current status
                const session =
                            await WhatsAppSessionRepository.getByInstanceIdAny(instanceName);
                ...
                if (session) {
                            ...
                            console.log(
                                          `[Webhook] connection.update → office ${session.officeId}: ${isOpen ? "connected" : "disconnected"}`,
                                        );
                } else {
                            // fallback: حاول عبر officeId القديم إن وُجد
                        const legacyOfficeId = instanceName.replace("office_", "");
                            const legacySession =
                                          await WhatsAppSessionRepository.getByOfficeId(legacyOfficeId);
                            if (legacySession) {
                                          await WhatsAppSessionRepository.updateStatus(
                                                          legacySession.id,
                                                          isOpen ? "connected" : "disconnected",
            ...
            if (payload.event === "messages.upsert") {
                ...
                if (!officeId) {
                            console.error(
                                          `[Webhook] No office found for instance: ${instanceName}`,
                                        );
                            return NextResponse.json({ ok: true });
                }
                ...
                if (!businessPhone) {
                            console.error(
                                          `[Webhook] No phone found for office: ${officeId}`,
                                        );
                            return NextResponse.json({ ok: true });
                }
